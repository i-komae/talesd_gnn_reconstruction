...
TALE-SD graph でも同じ考え方を使います。1 event には 2 種類の node と v2 の 7 種類の relation があります。
...
v2 schema では no-signal live detector の ``detector_waveform_valid`` は 0 です。
...
v2 の pulse-pulse relation は次のように分けます。

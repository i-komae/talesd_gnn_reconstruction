...
1. `dstio.tale.graph` が DST から `GraphEvent` を作り、graph relation、pulse selection、refill、HDF5 shard 書き込みを担当する
2. detector node と pulse node、v3の7種類の typed edge relation を持つ HDF5 graph を学習用cacheとして保存する
...
`dstio.tale.graph` に寄せます。GNN repo 側は CLI orchestration、dataset
読み込み、model input、training、direct inference、診断図を担当します。
graph relation、pulse cut、waveform duplication、balanced selection、refill を
GNN repo 側で勝手に変更しません。

```text
.venv/bin/talesd-gnn export-hetero
  -> cli._cmd_export_hetero()
    -> dstio.tale.graph.write_balanced_graph_h5()  # balanced MC export
       or dstio.tale.graph.write_graph_h5()        # unbalanced/streaming export
...
| hetero HDF5書き込み | `dstio.tale.graph.write_balanced_graph_h5`, `dstio.tale.graph.write_graph_h5`; 読み込み側は `src/talesd_gnn_reconstruction/hetero_graph_io.py` |
...
balanced MC export で `--energy-sample-per-bin` を付けた場合は、
`dstio.tale.graph.write_balanced_graph_h5` が source manifest scan、
`DAT??????` source group 管理、zenith-stratified split allocation、
event index の deterministic random selection、graphable 実数に基づく refill、
GraphEvent build、HDF5 shard write までを担当します。
...
各サイズは DST から直接作成されます。`20000` と `10000` は `50000` からの reshard/subsample ではありません。

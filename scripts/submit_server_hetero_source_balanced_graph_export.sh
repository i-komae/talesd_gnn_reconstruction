...
status "submit_server_hetero_source_balanced_graph_export.sh: starting"
...
SELECTION_SUMMARY="${SELECTION_SUMMARY:-${GRAPH_RUN_DIR}/summaries/hetero_source_balanced_selection_summary.json}"
...
if [[ -z "${CONST_DST}" ]]; then
  echo "CONST_DST is required for MC hetero source-balanced export." >&2
  exit 2
fi
...
echo "======================================================================"
echo "HETERO SOURCE-BALANCED GRAPH EXPORT"
echo "date=\$(date)"
...
echo "selection_strategy=filename_source_group_source_balanced_v1"
echo "======================================================================"
...
.venv/bin/python -m talesd_gnn_reconstruction.cli export-hetero-source-balanced \\
  --kind "mc" \\
  --const-dst "${CONST_DST}" \\
...
cat >&2 <<EOF
HETERO SOURCE-BALANCED GRAPH EXPORT SBATCH READY
run_name: ${RUN_NAME}

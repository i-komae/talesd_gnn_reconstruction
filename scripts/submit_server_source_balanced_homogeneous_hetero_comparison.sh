...
status "submit_server_source_balanced_homogeneous_hetero_comparison.sh: starting"
...
if ! [[ "${TARGET}" =~ ^[1-9][0-9]*$ ]]; then
  echo "TARGET must be a positive integer: ${TARGET}" >&2
  exit 2
fi
...
if [[ "${SOURCE_GROUP_SELECTION}" == "all" ]]; then
  ...
  H5_RUN_NAME="${H5_RUN_NAME:-hetero_source_balanced_all_target${TARGET}_${RUN_ID}}"
  MAX_SOURCE_GROUPS_PER_STRATUM="${MAX_SOURCE_GROUPS_PER_STRATUM:-}"
else
  SOURCE_SELECTION_TAG="balanced"
  COMPARISON_LABEL="BALANCED-SOURCE"
  H5_RUN_NAME="${H5_RUN_NAME:-hetero_source_balanced_min_target${TARGET}_${RUN_ID}}"
  MAX_SOURCE_GROUPS_PER_STRATUM="${MAX_SOURCE_GROUPS_PER_STRATUM:-}"
fi
...
MAX_SINGLE_JOB_TARGET="${MAX_SINGLE_JOB_TARGET:-100000}"
...
if ! [[ "${MAX_SINGLE_JOB_TARGET}" =~ ^[0-9]+$ ]]; then
...
MAX_SINGLE_JOB_TARGET only after checking the H5 export worker count,
...
status "max_single_job_target: ${MAX_SINGLE_JOB_TARGET}"
...
status "heterogeneous_note: final model choice requires validation/test on the new ${SOURCE_GROUP_SELECTION} H5"

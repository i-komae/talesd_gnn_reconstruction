...
if [[ "${TRAINING_BACKEND}" == "hetero" && "${SPEED_BENCHMARK}" == "1" ]]; then
...
BATCH_SIZE_WAS_SET=0
GRADIENT_ACCUMULATION_STEPS_WAS_SET=0
PIN_MEMORY_WAS_SET=0
PREFETCH_FACTOR_WAS_SET=0
PERSISTENT_WORKERS_WAS_SET=0
TRAIN_WORKERS_WAS_SET=0
PREPARE_FAST_CACHE_WAS_SET=0
if [[ -n "${BATCH_SIZE:-}" ]]; then
  BATCH_SIZE_WAS_SET=1
fi
if [[ -n "${GRADIENT_ACCUMULATION_STEPS:-}" ]]; then
  GRADIENT_ACCUMULATION_STEPS_WAS_SET=1
fi
if [[ -n "${PIN_MEMORY:-}" ]]; then
  PIN_MEMORY_WAS_SET=1
fi
if [[ -n "${PREFETCH_FACTOR:-}" ]]; then
  PREFETCH_FACTOR_WAS_SET=1
fi
if [[ -n "${PERSISTENT_WORKERS:-}" ]]; then
  PERSISTENT_WORKERS_WAS_SET=1
fi
if [[ -n "${TRAIN_WORKERS:-}" ]]; then
  TRAIN_WORKERS_WAS_SET=1
fi
...
if [[ "${TRAINING_BACKEND}" == "hetero" && "${WAVEFORM_ENCODER}" == "transformer" ]]; then
  if [[ "${BATCH_SIZE_WAS_SET}" == "0" ]]; then
    BATCH_SIZE=32
  fi
  if [[ "${GRADIENT_ACCUMULATION_STEPS_WAS_SET}" == "0" ]]; then
    GRADIENT_ACCUMULATION_STEPS=4
  fi
  if [[ "${PIN_MEMORY_WAS_SET}" == "0" ]]; then
    PIN_MEMORY=0
  fi
  if [[ "${PREFETCH_FACTOR_WAS_SET}" == "0" ]]; then
    PREFETCH_FACTOR=1
  fi
...
if [[ "${TRAINING_BACKEND}" == "hetero" ]]; then
  ...
  VALIDATE_EVERY_N_EPOCHS="${VALIDATE_EVERY_N_EPOCHS:-1}"
  MAX_VAL_GRAPHS="${MAX_VAL_GRAPHS:-}"
  HETERO_TRAINING_DATA_FORMAT="${HETERO_TRAINING_DATA_FORMAT:-pyg}"
  HETERO_RELATIONS="${HETERO_RELATIONS:-all}"
  HETERO_RELATION_PRESET="${HETERO_RELATION_PRESET:-}"
  USE_PULSE_PARENT_WAVEFORM="${USE_PULSE_PARENT_WAVEFORM:-1}"
  USE_PULSE_BOUNDS="${USE_PULSE_BOUNDS:-1}"
  PULSE_WAVEFORM_ENCODER="${PULSE_WAVEFORM_ENCODER:-bounds}"
  USE_RELATIVE_POSITIONS="${USE_RELATIVE_POSITIONS:-0}"
...
echo "train_loader_memory_budget_gib=${TRAIN_LOADER_MEMORY_BUDGET_GIB}"
...
echo "checkpoint_milestones=${CHECKPOINT_MILESTONES}"
echo "checkpoint_milestone_full_eval=${CHECKPOINT_MILESTONE_FULL_EVAL}"
echo "allow_train_loss_checkpoint=${ALLOW_TRAIN_LOSS_CHECKPOINT}"
echo "milestone_eval_epochs=${MILESTONE_EVAL_EPOCHS}"
echo "milestone_eval_split=${MILESTONE_EVAL_SPLIT}"
echo "milestone_eval_max_graphs=${MILESTONE_EVAL_MAX_GRAPHS}"
echo "milestone_eval_current_model=${MILESTONE_EVAL_CURRENT_MODEL}"
echo "milestone_eval_best_model=${MILESTONE_EVAL_BEST_MODEL}"
echo "milestone_eval_diagnostics=${MILESTONE_EVAL_DIAGNOSTICS}"
echo "speed_benchmark=${SPEED_BENCHMARK}"
echo "profile=${PROFILE}"
echo "final_eval_data_format=${FINAL_EVAL_DATA_FORMAT}"
echo "core_target_mode=${CORE_TARGET_MODE:-}"
echo "coordinate_feature_mode=${COORDINATE_FEATURE_MODE:-}"
...
echo "waveform_transformer_heads=${WAVEFORM_TRANSFORMER_HEADS}"
echo "waveform_transformer_layers=${WAVEFORM_TRANSFORMER_LAYERS}"
echo "waveform_transformer_max_tokens=${WAVEFORM_TRANSFORMER_MAX_TOKENS}"
echo "waveform_transformer_downsample=${WAVEFORM_TRANSFORMER_DOWNSAMPLE}"
echo "use_pulse_parent_waveform=${USE_PULSE_PARENT_WAVEFORM}"
echo "use_pulse_bounds=${USE_PULSE_BOUNDS}"
echo "pulse_waveform_encoder=${PULSE_WAVEFORM_ENCODER}"
echo "use_relative_positions=${USE_RELATIVE_POSITIONS}"
echo "detector_readout_mask=${DETECTOR_READOUT_MASK}"
echo "pulse_readout_mask=${PULSE_READOUT_MASK}"
...
echo "energy_bias_bin_width=${ENERGY_BIAS_BIN_WIDTH}"
...
echo "feature_importance_split=${FEATURE_IMPORTANCE_SPLIT}"
echo "attention_maps=${ATTENTION_MAPS}"
echo "source_fraction_mode=${SOURCE_FRACTION_MODE}"
echo "attention_maps_split=${ATTENTION_MAPS_SPLIT}"
...
echo "validate_every_n_epochs=${VALIDATE_EVERY_N_EPOCHS}"
echo "max_val_graphs=${MAX_VAL_GRAPHS}"
echo "homogeneous_schema=${HOMOGENEOUS_SCHEMA}"
echo "hetero_training_data_format=${HETERO_TRAINING_DATA_FORMAT}"
echo "hetero_relation_preset=${HETERO_RELATION_PRESET:-}"
echo "hetero_relations=${HETERO_RELATIONS}"
echo "dataloader_timeout_sec=${DATALOADER_TIMEOUT_SEC}"
echo "data_wait_warn_sec=${DATA_WAIT_WARN_SEC}"
...
env \\
  ...
  PROFILE="${PROFILE}" \\
  TALESD_GNN_PROFILE="${TALESD_GNN_PROFILE:-${PROFILE}}" \\

...
RUN_NAME="${RUN_NAME:-${TAG_PREFIX}_${RUN_ID}}"
...
printf "%s\n" "${ACTIVE_CONFIGS[@]}" > "${CONFIG_DIR}/configs.txt"
...
run_one() {
  ...
  local checkpoint="${CHECKPOINT_DIR}/${cfg_tag}.pt"
  local log_path
...
job_log_path_for_config() {
  ...
  local cfg_tag _hidden_dim _layers _lr _dropout _weight_decay _lr_scheduler
  IFS='|' read -r cfg_tag _hidden_dim _layers _lr _dropout _weight_decay _lr_scheduler <<< "${config}"
  printf "%s/%s.log" "${LOG_DIR}" "${cfg_tag}"
}
...
for config in "${ACTIVE_CONFIGS[@]}"; do
  IFS='|' read -r cfg_tag _hidden_dim _layers _lr _dropout _weight_decay _lr_scheduler <<< "${config}"
  metrics_path="${CHECKPOINT_DIR}/${cfg_tag}.pt.metrics.json"

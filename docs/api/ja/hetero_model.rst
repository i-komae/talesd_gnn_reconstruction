...
v3 schema の ``detector_trigger_usec_rel`` は互換性のための名前です。
値は detector node の代表到着時刻として使います。
具体的には、その detector に属する最初の有効 pulse の立ち上がり時刻を、
event 内の最初の有効 pulse からの相対時刻にしたものです。
detector waveform の開始時刻としては使いません。
no-signal live detector は ``detector_arrival_time_valid = 0`` で、この列は 0 です。

...
``_reconstruction_loss`` : ``train.py:691`` が再構成lossです。
...
``_quality_targets_from_reconstruction`` : ``train.py:889`` が、再構成誤差からsoft quality targetを作ります。
``_quality_prediction_loss`` : ``train.py:915`` はquality logitとsoft targetのBCEWithLogitsです。
...
``_mass_classification_loss`` : ``train.py:1017`` がmass分類lossです。
``bce`` と ``focal`` を扱います。

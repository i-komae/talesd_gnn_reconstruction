...
class _FakeGraphDataset:
    def __init__(self, source_counts: dict[str, int]):
        ...
        for source_path, count in source_counts.items():
            ...
            zenith_deg = 0.0
            if "_z" in source_path:
                token = source_path.split("_z", maxsplit=1)[1].split("_", maxsplit=1)[0].split(".", maxsplit=1)[0]
                zenith_deg = float(token)
            theta = np.deg2rad(zenith_deg)
            target = np.asarray(
                [loge, 0.0, 0.0, 0.0, np.sin(theta), 0.0, np.cos(theta)],
                dtype=np.float32,
            )
...
class SourceSplitTest(unittest.TestCase):
    def test_assign_source_group_splits_random_source_groups_not_event_counts(self) -> None:
        ...
        self.assertEqual(len(split_sources["test"]), 2)
        self.assertEqual(len(split_sources["val"]), 1)
        self.assertEqual(len(split_sources["train"]), 7)

    def test_assign_source_group_keeps_event_counts_as_consequence(self) -> None:
        ...
        self.assertEqual(
            sum(counts[source_path] for source_path in counts),
            sum(counts[source_path] for sources in split_sources.values() for source_path in sources),
        )
    ...
    def test_stratified_source_split_randomizes_within_particle_energy_zenith(self) -> None:
        source_counts = {}
        for zenith in (10, 40):
            for index in range(6):
                source_counts[f"/mc/proton/DAT{zenith:02d}{index:02d}16_z{zenith}_gea_trg_000.dst.gz"] = 3
        dataset = _FakeGraphDataset(source_counts)
        ...
        split = split_indices_by_stratified_source_path(
            ...
            show_progress=False,
            workers=0,
            source_val_fraction=1.0 / 3.0,
            source_test_fraction=1.0 / 3.0,
        )
        ...
        for zenith in (10, 40):
            for name in ("train", "val", "test"):
                self.assertEqual(
                    sum(1 for source_path in split_source_paths[name] if f"_z{zenith}_" in source_path),
                    2,
                )
    ...
    def test_corsika_split_dst_chunks_stay_in_same_split(self) -> None:
        ...
        for dat_index in range(20):
            ...
            for chunk_index in range(3):
                source_counts[
                    f"/mc/{particle}/bin_{loge}/DAT{dat_index:04d}{loge:02d}_gea_trg_{chunk_index:03d}.dst.gz"
                ] = 4
        dataset = _FakeGraphDataset(source_counts)

...
from talesd_gnn_reconstruction.cli import (
    ...
    _write_light_hetero_graph_h5,
)
import talesd_gnn_reconstruction.cli as cli_module
...
class SyntheticHeteroGraphIoTest(unittest.TestCase):
    ...
    def test_light_hetero_writer_splits_large_stratum_work_and_aggregates_summary(self) -> None:
        groups = [
            LightHeteroSourceGroup(
        ...
        self.assertEqual(result["requested_workers"], 4)
        self.assertEqual(result["workers"], 4)
        self.assertEqual(len(seen_payloads), 4)
        self.assertEqual(len(result["stratum_work_items"]), 4)
        self.assertEqual(len(result["strata"]), 1)
        self.assertEqual(result["strata"][0]["stratum"], "proton:16")
        self.assertEqual(result["strata"][0]["target_graphs"], 80)
        self.assertEqual(result["strata"][0]["graphs_written"], 80)
        self.assertEqual(result["strata"][0]["candidate_source_groups"], 8)
    ...
    def test_light_hetero_worker_records_underfull_stratum(self) -> None:
                    ...
                    "groups": [
                        {
                            ...
                            "particle": "iron",
                            "stratum": "iron:40",
                            "paths": ["/mc/iron/short_a.dst.gz"],
                        },
                        {
                            "source_group": "/mc/iron/DAT000240",
                            "dat_tag": "DAT000240",
                            "energy_bin_code": "40",
                            "particle": "iron",
                            "stratum": "iron:40",
                            "paths": ["/mc/iron/short_b.dst.gz"],
                        },
                    ],
                }
            ],
        ...
        self.assertEqual(result["graphs_written"], 7)
        self.assertEqual(result["strata"][0]["graphs_written"], 7)
        self.assertEqual(result["strata"][0]["target_graphs"], 20)
        self.assertEqual(result["strata"][0]["target_met"], False)
...
class HeteroGraphIoTest(unittest.TestCase):
    ...
    def test_hetero_attention_maps_smoke_writes_json_and_npz(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertTrue(Path(result["plot_pdf"]).exists())
            payload = json.loads(summary_path.read_text())
            self.assertEqual(payload["format"], "hetero_attention_maps_v1")
            self.assertEqual(payload["n_graphs"], 1)
            self.assertTrue(Path(payload["plot_pdf"]).exists())
